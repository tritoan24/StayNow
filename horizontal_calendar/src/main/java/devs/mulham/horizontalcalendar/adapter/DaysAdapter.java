...
public class DaysAdapter extends HorizontalCalendarBaseAdapter<DateViewHolder, Calendar> {
    ...
    public void onBindViewHolder(DateViewHolder holder, int position) {
        ...
        if (config.isShowBottomText()) {
            if (todayCalendar.get(Calendar.YEAR) == day.get(Calendar.YEAR) && todayCalendar.get(Calendar.MONTH) == day.get(Calendar.MONTH) && todayCalendar.get(Calendar.DAY_OF_MONTH) == day.get(Calendar.DAY_OF_MONTH)) {
                holder.textBottom.setText(R.string.today);
            } else {
