...
public final class HorizontalCalendar {
    ...
    void init(View rootView, final Calendar defaultSelectedDate, HorizontalCalendarPredicate disablePredicate, CalendarEventsPredicate eventsPredicate) {
        ...
        if (calendarView == null) {
            throw new IllegalStateException("HorizontalCalendarView not found in the layout!");
        }
        ...
        post(new Runnable() {
            @Override
            public void run() {
                centerToPositionWithNoAnimation(false, positionOfDate(defaultSelectedDate));
            }
        });
    ...
    public void selectDate(Calendar date, boolean immediate) {
        ...
        if (immediate) {
            centerToPositionWithNoAnimation(immediate, datePosition);
     ...
     * @param position The position to center the calendar to!
     */
    void centerToPositionWithNoAnimation(boolean notifyAllData, final int position) {
        if (position != -1) {
            ...
            calendarView.post(new Runnable() {
                ...
                public void run() {
                    ...
                    refreshItemsSelector(notifyAllData, newSelectedItem, oldSelectedItem);
                }
            });
        }
    }

    void refreshItemsSelector(boolean notifyAllData, int position1, int... positions) {
        if (notifyAllData) {
        ...
        private class SelectedItemRefresher implements Runnable {
            ...
            public void run() {
                ...
                if ((lastSelectedItem == -1) || (lastSelectedItem != positionOfCenterItem)) {
                    //On Scroll, agenda is refresh to update background colors
                    refreshItemsSelector(false, positionOfCenterItem);
                    if (lastSelectedItem != -1) {
                        refreshItemsSelector(false, lastSelectedItem);
                    }
